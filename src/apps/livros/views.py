...
def index(request, page=1):
    ...
    livros = Livro.objects.filter(title__icontains=artigo_busca).order_by("title")[
        start_index:end_index
    ]
    total_items = Livro.objects.filter(title__icontains=artigo_busca).count()
